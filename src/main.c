...
void test_heap()
{
    ...
    push_heap(heap, 66);
    CU_ASSERT(peek_heap(heap) == 66);
    push_heap(heap, 89);
    CU_ASSERT(peek_heap(heap) == 89);
    CU_ASSERT(pop_heap(heap) == 89);    // pop the root value
    CU_ASSERT(replace(heap, 10) == 10); // pop root and push a new key.
